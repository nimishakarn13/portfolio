...
export function createDisplacement(
  ...
  // Browsers without the experimental html-in-canvas capture (drawElementImage)
  // can't rasterize arbitrary live HTML, but a static <img> inside `content`
  // can still be uploaded straight into the WebGL texture via texImage2D.
  ...
  function syncCanvasSize() {
    ...
    if (htmlInCanvas) {
      ...
      // `source` is otherwise idle in this branch (it's display:none), so
      // reuse it as the offscreen canvas we rasterize the fallback image into.
  ...
  if (fallbackImage) {
    fallbackImage.addEventListener("load", paintFallbackImage);
  }
  ...
  return {
    ...
    destroy() {
      ...
      fallbackImage?.removeEventListener("load", paintFallbackImage);
    },
  };
}
