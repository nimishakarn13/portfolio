...
export interface SplashCursorProps {
  ...
  /** Documented for API parity with the source component; the fluid's clear color is hardcoded internally and never reads this. */
  BACK_COLOR?: RGBColor
  TRANSPARENT?: boolean
  ...
  useEffect(() => {
    ...
    function pointerPrototype() {
      return {
        ...
        down: false,
        moved: false,
        color: [0, 0, 0] as number[] | RGBColor,
      }
    }
    ...
    // The internal WebGL machinery below (framebuffer objects, shader
    // programs, uniform dictionaries) is dynamically shaped — typed loosely
    // as `any` throughout rather than modeled precisely, matching how this
    // kind of ported WebGL demo is conventionally handled in TS.
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    function getUniforms(program: any): Record<string, WebGLUniformLocation> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    function compileShader(type: number, source: string, keywords?: string[] | null): any {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: number[] | RGBColor) {
      const c = Array.isArray(color) ? { r: color[0] ?? 0, g: color[1] ?? 0, b: color[2] ?? 0 } : color
      splatProgram.bind()
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: number[] | RGBColor) {
      pointer.prevTexcoordX = pointer.texcoordX
      pointer.prevTexcoordY = pointer.texcoordY
      pointer.texcoordX = posX / canvas!.width
      pointer.texcoordY = 1.0 - posY / canvas!.height
